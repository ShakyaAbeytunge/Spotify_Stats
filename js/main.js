...
// Create scales
const xScale = d3.scaleLinear().domain([0, totalDays]).range([0, sliderWidth]);
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const selectedCountry = urlParams.get("country"); // Populate the dropdown
  const countryDropdown = document.getElementById("country-list");
  ...
  // If a country is selected from the URL, set it as the selected value
  if (selectedCountry) {
    // Try to find the matching country in the list
    const matchingOption = Array.from(countryDropdown.options).find(
      (option) => option.value === selectedCountry
    );

    if (matchingOption) {
      countryDropdown.value = selectedCountry; // Set the dropdown value
    } else {
      console.warn(`Country "${selectedCountry}" is not in the dropdown.`);
    }
  }
});
...
function updateCountrylist() {
  ...
  ctx.countries.forEach((country) => {
    countryList.append("option").attr("value", country).text(country);
  });
...
tabs.forEach((tab) => {
  tab.addEventListener("click", function () {
    ...
    if (
      this.dataset.tab === "artist-analysis" &&
      !isArtistDropdownInitialized
    ) {
      createArtistDropdown("data/top_artists.csv");
      isArtistDropdownInitialized = true; // Prevent re-initialization
    } else if (
      this.dataset.tab === "song-analysis" &&
      !isSongDropdownInitialized
    ) {
...
function createSongDropdown(csvFilePath) {
  const container = d3
    .select("#song-analysis")
    .append("div")
    .attr("class", "artist-container");

  const leftPanel = container.append("div").attr("class", "artist-left-panel");

  leftPanel.append("label").attr("for", "songDropdown").text("Select Songs: ");

  const dropdown = leftPanel
    .append("select")
  ...
  const addButton = leftPanel
    .append("button")
    .attr("class", "addButton")
    .text("Add Song");

  const songListContainer = leftPanel
    .append("div")
  ...
  function renderSongList() {
    ...
    ctx.addedSongs.forEach((songKey) => {
      const song = songKey.split("-").slice(1).join("-");
      const tag = songListContainer
        .append("div")
  ...
  function addSong() {
    ...
    const selectedSongName =
      dropdown.node().options[dropdown.node().selectedIndex].text;

    if (
      selectedSongId &&
      !ctx.addedSongs.has(`${selectedSongId}-${selectedSongName}`)
    ) {
...
function updateSongPlot() {
  ...
  const allAggregatedData = addedSongsArray
    .map((song) => {
  ...
  const content = chart.append("g").attr("clip-path", "url(#clip)");
    ...
    path
      ...
      .on("end", () => {
        ...
        const circles = content
          .selectAll(`.circle-${sanitizeClassName(song)}`)
          .data(data)
          .enter()
          .append("circle")
  ...
  const zoom = d3
    ...
    .on("zoom", (event) => {
      ...
      xAxis.call(
        d3.axisBottom(newXScale).ticks(5).tickFormat(d3.timeFormat("%b %Y"))
      );

      content.selectAll("path").attr("d", (d) =>
        d3
          .line()
      ...
      content.selectAll("circle").attr("cx", (d) => newXScale(d.date));
    });
...
function createArtistDropdown(csvFilePath) {
  ...
  const artistListContainer = leftPanel
    .append("div")
    .attr("class", "listContainer");
  ...
  d3.csv(csvFilePath)
    .then((data) => {
      ...
      dropdown
        .selectAll("option.artist-option")
        .data(artistNames)
        .enter()
        .append("option")
    ...
    if (selectedArtist && !ctx.addedArtists.has(selectedArtist)) {
      if (ctx.addedArtists.size < ctx.maxOptions) {
        ...
        const songIDs = ctx.songData
          .filter((d) =>
            d.artist.toLowerCase().includes(selectedArtist.toLowerCase())
          )
          .map((d) => d.track_id);
...
function updateArtistPlot() {
  ...
  const allAggregatedData = addedArtistsArray
    .map((artist) => {
      const songIds = ctx.artistSongs[artist] || [];
      const artistData = ctx.chartData.filter((d) =>
        songIds.includes(d.track_id)
      );
  ...
  artistGroups.forEach(([artist, data]) => {
    ...
    path
      ...
      .on("end", () => {
        content
          .selectAll(`circle-${artist.replace(/\s+/g, "")}`)
          .data(data)
          .enter()
          .append("circle")
          .attr("cx", (d) => xScale(d.date))
...
function updateAttributeAnalysis() {
  ...
  const songDataMap = new Map(
    ctx.songData.map((song) => [song.track_id, song])
  );
  ...
  const monthlyData = d3.rollups(
    ...
    (v) => {
      ...
      return Object.fromEntries(
        attributes.map((attr, i) => [attr, attributeAverages[i]])
      );
    },
  ...
  const legend = svg
    ...
    .attr(
      "transform",
      `translate(${margin.left + width / 2 - legendWidth / 2}, ${
        svgHeight - 50
      })`
    );
